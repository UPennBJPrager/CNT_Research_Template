...
for _, row in metadata.iterrows():
    ...
    for portal_name in iEEG_portal_names:
        ...
        for i in tqdm(range(n_iter)):
            ...
            data, fs = tools.get_iEEG_data(username, pwd_bin_path, portal_name, start_usec, end_usec, select_electrodes=channel_labels)
            ...
            for name, band in zip(band_names, bands):
                ...
                for ind, win_inds in enumerate(sl_win):
                    power_mat[ind, :] = tools.bandpower(signal_ref.iloc[win_inds, :], fs, band, 1, relative=True)
