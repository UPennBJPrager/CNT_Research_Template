...
def bb_moments_raincloud(region_idx=None, parcellation='aparc', title=''):
    ...
    bb_pth = '/Users/sl1063/repos/ENIGMA/enigmatoolbox/histology/bb_moments_' + parcellation + '.csv'
    bb_moments_aparc = np.loadtxt(bb_pth, delimiter=',', dtype=float)
    bb_moments_aparc[0, :] = NormalizeData(bb_moments_aparc[0, :])
    bb_moments_aparc[1, :] = NormalizeData(bb_moments_aparc[1, :])
    ...
    for ii in range(bb_moments_aparc.shape[0]):
        ...
        rando = [(random.random() * .3) + (jj - 0.15) for rr in range(bb_moments_aparc[ii, region_idx].shape[0])]
        ...
        data = sns.kdeplot(bb_moments_aparc[ii, region_idx], linewidth=0, ax=axs2[ii]).get_lines()[0].get_data()
        ...
        mad = 3 * median_abs_deviation(bb_moments_aparc[ii, region_idx], axis=0)
